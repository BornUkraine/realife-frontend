    ...
    >
      <div
        className={[
          ...
          "bg-[#0b0a09]/72 backdrop-blur-2xl",
...
function SidebarNavItem({
  ...
  const disabledCls = cx(
    "cursor-not-allowed select-none opacity-45",
...
function SidebarFooterCard() {
  return (
    <div className="rounded-[28px] border border-white/10 bg-white/[0.04] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.25)]">
...
export default function AppSidebar({
  ...
  return (
    <div className="lg:sticky lg:top-6 xl:top-8">
      <GoldEdgeCard>
        <div className="flex flex-col p-5 xl:p-6 lg:min-h-[calc(100vh-3.5rem)]">
          <Link
            href="/app"
          ...
          <div className="mt-5 rounded-3xl border border-white/10 bg-white/[0.04] p-4 shadow-[0_18px_60px_rgba(0,0,0,0.22)]">
            <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-white/42">
              Wallet
            </div>
            <div className="mt-2 text-[12px] leading-relaxed text-white/52">
              Connect to mint, trade, and access delivery-aware ownership flows.
            </div>
          </div>

          <div className="mt-auto pt-5">
            {bottom ? <div className="mb-4">{bottom}</div> : null}
            <SidebarFooterCard />
          </div>
        </div>
      </GoldEdgeCard>
