// PATH: components/nft/NftDetail.tsx
// Shared NFT detail body — rendered by BOTH the full page (/app/nft/...)
// and the OpenSea-style intercepting modal. variant="page" keeps the
// heavy full-screen background; variant="modal" drops it (the gallery is
// already painted behind the modal).
...
export type NftDetailVariant = "page" | "modal";
...
export default async function NftDetail({
  ...
  tokenIdParam: string;
  variant?: NftDetailVariant;
}) {
  const chainId = Number(chainIdParam);
  const contract = norm(safeDecode(contractParam || ""));
  const tokenId = safeDecode(tokenIdParam || "").trim();
  ...
  return (
    <main
      className={cx(
        ...
        isModal
          ? "min-h-0"
          : "min-h-screen overflow-x-hidden bg-[#060505]"
      )}
    >
      {!isModal ? (
