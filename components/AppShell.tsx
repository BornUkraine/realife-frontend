...
export default function AppShell({
  ...
  // Получаем текущий путь, чтобы анимация срабатывала при каждом переходе по меню
    ...
    <main className="min-h-screen bg-[#070606] text-white overflow-x-hidden">
      ...
      <div className="relative max-w-7xl mx-auto px-6 md:px-10 pt-10 pb-10 md:pt-14 md:pb-14 grid grid-cols-12 gap-10">
        ...
        <section className="col-span-12 lg:col-span-9 space-y-6">
          {/* 🔥 Обертка с анимацией. Ключ (key) заставляет её перезапускаться при смене страницы */}
          <motion.div
            key={pathname}
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.6, ease: [0.16, 1, 0.3, 1] }} // Дорогая, плавная пружинная кривая
          >
            {children}
          </motion.div>
        </section>
