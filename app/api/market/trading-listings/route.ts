...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (requestedMarketType && contract) {
      if (PUBLIC_DELIVERY_CONTRACT && contract === PUBLIC_DELIVERY_CONTRACT) {
        // Delivery contract is protected by product logic.
      } else if (CAFE_CONTRACT && contract === CAFE_CONTRACT) {
        // Cafe remains standard resale.
      } else if (STORE_CONTRACT && contract === STORE_CONTRACT) {
        // Store remains standard resale.
      } else {
        where.marketType = requestedMarketType;
      }
    }
    ...
    return NextResponse.json({
      ok: true,
      mode: "trading-listings-fallback",
    ...
    console.error("[API_TRADING_LISTINGS_FALLBACK_ERROR]", e);

    return NextResponse.json(
      {
        ok: false,
        error: "TRADING_LISTINGS_FALLBACK_FAILED",
        message: e?.message || "Trading listings API failed",
      },
      { status: 500 }
    );
  }
