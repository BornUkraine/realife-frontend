...
function Pill({ children }: { children: ReactNode }) {
  return (
    <div className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 text-[11px] font-semibold text-white/60 backdrop-blur-2xl shadow-[0_12px_40px_rgba(0,0,0,0.25)]">
      {children}
    </div>
  );
}

function GoldEdgeWrap({ className = "", children }: { className?: string; children: ReactNode }) {
  return (
    <div className={cx(
...
function Chip({ children, tone = "muted" }: { children: ReactNode; tone?: "muted" | "gold" | "brand" }) {
  const cls =
    tone === "gold"  ? "border-amber-500/25 bg-amber-500/10 text-amber-100" :
    tone === "brand" ? "border-white/15 bg-white/[0.08] text-white/75" :
                       "border-white/10 bg-white/[0.06] text-white/60";
  return (
...
function KeyValue({ label, value, mono = false }: { label: string; value: ReactNode; mono?: boolean }) {
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-4">
      ...
      <div className={cx("mt-1 text-sm font-bold text-white/85 truncate", mono && "font-mono text-[13px]")}>
        {value}
      </div>
    </div>
  );
...
const userSelect = {
  id: true, handle: true, publicId: true, points: true,
  walletAddress: true, walletChainId: true,
  twitterId: true, twitterUser: true, twitterName: true, twitterImage: true,
  discordId: true, discordUser: true, discordName: true, discordImage: true,
} as const;
...
export default async function PublicProfilePage({
  ...
  const nftCount = await prisma.holding.count({
    where: { userId: user.id, amount: { gt: 0n }, mint: { verified: true } },
  });
  ...
  const displayName =
    user.twitterName || user.discordName || xHandle || dcHandle ||
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-10 space-y-8">
                ...
                <div className="mt-6 flex flex-wrap gap-3">
                  {xUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex items-center justify-center px-5 py-2.5 rounded-2xl border border-white/15 bg-white/[0.06] font-bold backdrop-blur-2xl shadow-[0_18px_70px_rgba(0,0,0,0.28)] hover:bg-white/10 hover:-translate-y-px transition active:translate-y-0"
                    >
                      Open X ↗
                    </a>
                  )}
                  {publicUrl && (
                    <Link
                      href={publicUrl}
                      className="inline-flex items-center justify-center px-5 py-2.5 rounded-2xl text-black font-extrabold hover:brightness-110 transition shadow-[0_18px_60px_rgba(212,175,55,0.20)] bg-[linear-gradient(135deg,#f7e7a7_0%,#d4af37_45%,#b8870a_100%)] ring-1 ring-black/15"
                    >
                      Public URL
                    </Link>
                  )}
                  {nftsUrl && (
                    <Link
                      href={nftsUrl}
                      className="inline-flex items-center justify-center px-5 py-2.5 rounded-2xl text-black font-extrabold bg-[linear-gradient(135deg,#f7e7a7_0%,#d4af37_45%,#b8870a_100%)] shadow-[0_18px_60px_rgba(212,175,55,0.18)] ring-1 ring-black/15 hover:brightness-110 hover:-translate-y-px active:translate-y-0 transition"
                    >
                      NFTs →
                    </Link>
                  )}
        ...
        <div className="reveal grid md:grid-cols-2 gap-6" style={{ animationDelay: "110ms" }}>
          ...
          <Card>
            ...
            {twitterConnected ? (
              <div className="flex items-center gap-4 bg-white/[0.03] p-4 rounded-2xl border border-white/[0.07]">
                <Avatar src={user.twitterImage} fallback="X" size="md" />
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-bold truncate text-white/90">{user.twitterName || "—"}</div>
                  <div className="text-xs text-white/40 font-mono truncate">@{user.twitterUser}</div>
                </div>
                {xUrl && (
                  <a href={xUrl} target="_blank" rel="noreferrer" className="shrink-0 text-xs font-semibold text-[#C9A84C] hover:brightness-110 transition">
                    View ↗
                  </a>
                )}
              </div>
              ...
              <div className="relative overflow-hidden rounded-2xl border border-white/[0.07] bg-white/[0.03] p-4">
                ...
                <div className="relative z-10 flex items-center gap-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="text-sm font-bold truncate text-white/75">Not connected</div>
                    <div className="text-xs text-white/40 mt-0.5">This user hasn't linked X yet.</div>
                  </div>
                  <div className="shrink-0"><Chip>🔒 Private</Chip></div>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
          ...
          <Card>
            ...
            {discordConnected ? (
              <div className="flex items-center gap-4 bg-white/[0.03] p-4 rounded-2xl border border-white/[0.07]">
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-bold truncate text-white/90">{user.discordName || "—"}</div>
        ...
        <footer
          className="reveal pt-6 text-[10px] font-bold text-white/20 text-center uppercase tracking-[0.4em]"
