        ...
        <div className="relative mx-auto max-w-6xl px-6 py-10 space-y-6">
          <Card>
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              ...
              <div className="w-full md:w-[360px] space-y-3">
                {!authed ? (
                  <GoldBtn onClick={() => signIn("twitter")}>Connect X (Login)</GoldBtn>
                ) : (
                  <GhostBtn onClick={() => signOut()}>Logout</GhostBtn>
                )}
          ...
          <div className="grid md:grid-cols-2 gap-6">
            <Card>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-white/60 mt-1">
                    Shows your name/username automatically.
                  </div>
                </div>
                <div className={`text-[11px] font-semibold px-3 py-1.5 rounded-full border ${
                  twitterConnected
              ...
              <div className="mt-4 flex items-center gap-4">
                <div className="h-14 w-14 rounded-2xl border border-white/10 bg-white/[0.06] overflow-hidden flex items-center justify-center">
                  {me?.twitterImage ? (
                    ...
                    <img src={me.twitterImage} alt="X" className="h-full w-full object-cover" />
                  ) : (
              ...
              <div className="mt-5">
                <GoldBtn disabled={!authed} onClick={() => signIn("twitter")}>
                  {twitterConnected ? "Re-connect X" : "Connect X (+100)"}
                </GoldBtn>
              </div>
            </Card>
            ...
            <Card>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-white/60 mt-1">
                    Links Discord to the same profile.
                  </div>
                </div>
                <div className={`text-[11px] font-semibold px-3 py-1.5 rounded-full border ${
                  discordConnected
              ...
              <div className="mt-4 flex items-center gap-4">
                <div className="h-14 w-14 rounded-2xl border border-white/10 bg-white/[0.06] overflow-hidden flex items-center justify-center">
                  {me?.discordImage ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={me.discordImage} alt="Discord" className="h-full w-full object-cover" />
                  ) : (
                    <span className="text-white/40 text-xs">DS</span>
                  )}
                </div>
              ...
              <div className="mt-5">
                <GoldBtn disabled={!authed} onClick={() => signIn("discord")}>
                  {discordConnected ? "Re-connect Discord" : "Connect Discord (+100)"}
                </GoldBtn>
              </div>
            </Card>
          </div>
